...
def getForm(request):
    if not request.method == "POST":
        return HttpResponse(
            f'You are in the wrong page, click <a href="{reverse("index")}">here</a> here to go to the homepage'
        )
    ...
    if template.exists():
        ...
        return JsonResponse(
            {
                "message": f'Invalid code, click <a href="{reverse("index")}">here</a> here to try again'
            },
            status=400,
        )
...
def reportView(request):
    if not request.user.is_authenticated:
        return HttpResponse(
            f'You are not authorized to access this page, click <a href="{reverse("index")}">here</a> here to go to the homepage'
        )

    code = request.GET.get("code")

    return render(request, "report.html", {"code": code, "link": reverse("report")})
...
def getReport(request):
    ...
    if template.exists():
        ...
        return HttpResponse(
            f'Invalid code, click <a href="{reverse("report")}">here</a> here to try again'
        )
